...
QEMU can emulate a great variety of hardware, from ARM to Sparc. On x86 hosts,
{pve} is concerned with 32-bit and 64-bit PC clone emulation, since it
represents the overwhelming majority of server hardware. On arm64 hosts, {pve}
emulates 64-bit ARM (aarch64) machines instead. Emulating the same architecture as the host is also one of the
fastest options, due to the availability of processor extensions which greatly
speed up QEMU when the emulated architecture matches the host architecture.
...
xref:qm_pci_passthrough[PCIe passthrough]. On arm64 hosts OVMF is the default
and only option, as SeaBIOS is x86-only.
...
NOTE: The Intel 440FX and Q35 chipsets are specific to x86 guests. On arm64
hosts, virtual machines use the `virt` machine type, which models a generic,
UEFI-based ARM machine.
...
NOTE: The 'kvm64' and 'x86-64-v*' CPU models described here are x86-only. On
arm64 hosts, guests use ARM CPU models: 'host' to expose the physical CPU
(recommended on the supported hardware), or a generic model such as 'max',
together with the `virt` machine type.
...
NOTE: SeaBIOS, and thus `bios=seabios`, is available for x86 guests only. On
arm64 hosts, virtual machines always boot through UEFI, using the ARM build of
OVMF (AAVMF), so `bios=ovmf` is the only supported setting there.
...
For Windows, see our https://pve.proxmox.com/wiki/Windows_VirtIO_Drivers[Windows
VirtIO Drivers] article for more information on how to acquire and install the
drivers.
...
by disabling the `freeze-fs` guest agent option. This can also be done in the
GUI: open the 'Options' panel of the virtual machine, then the 'QEMU Guest
Agent' dialog, and disable the 'Freeze/thaw guest filesystems during certain
operations for consistency' option.
+
...
NOTE: Live migration is only possible between hosts of the same CPU
architecture. A running guest's CPU state cannot be transferred between an x86
and an arm64 host.
